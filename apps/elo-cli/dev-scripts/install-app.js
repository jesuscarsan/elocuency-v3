...
if (!availableApps || !Array.isArray(availableApps)) {
	console.warn(
		'Warning: "availableApps" is missing or not an array in elo-config-dev.json. Proceeding with provided app name.',
	);
} else {
	...
	if (availableApps.includes(appName)) {
		...
		console.log(
			`Note: '${appName}' is not in the 'availableApps' list in elo-config-dev.json, but will attempt to install from sourceUrl.`,
		);
	}
}
