...
var require_CommandIds = __commonJS({
  "../../libs/core-ts/dist/Domain/Constants/CommandIds.js"(exports) {
    ...
    (function(CommandEnum2) {
      CommandEnum2["ApplyTemplate"] = "ApplyTemplateCommand";
      CommandEnum2["ApplyTemplateWithUrl"] = "ApplyTemplateWithUrlCommand";
      CommandEnum2["ApplyStreamBrief"] = "ApplyStreamBriefCommand";
      CommandEnum2["EnhanceByAi"] = "EnhanceByAiCommand";
      CommandEnum2["AddImages"] = "AddImagesCommand";
      CommandEnum2["CreateNoteFromImages"] = "CreateNoteFromImagesCommand";
      CommandEnum2["ApplyTemplateFromImage"] = "ApplyTemplateFromImageCommand";
      CommandEnum2["GenerateHeaderMetadata"] = "generate-header-metadata";
      CommandEnum2["RelocateNoteByLinkField"] = "RelocateNoteByLinkFieldCommand";
      CommandEnum2["GenerateMissingNotesFromLinks"] = "GenerateMissingNotesFromLinksCommand";
      CommandEnum2["CreateReciprocityLinksNotes"] = "CreateReciprocityLinksNotesCommand";
      CommandEnum2["AnalyzeAndLinkEntities"] = "AnalyzeAndLinkEntitiesCommand";
      CommandEnum2["GenerateMissingNotesFromListField"] = "GenerateMissingNotesFromListFieldCommand";
      CommandEnum2["EnrichPlace"] = "EnrichPlaceCommand";
      CommandEnum2["RelocatePlaceNote"] = "RelocatePlaceNoteCommand";
      CommandEnum2["AddPlaceIdFromUrl"] = "AddPlaceIdFromUrlCommand";
      CommandEnum2["SearchSpotifyTrack"] = "SearchSpotifyTrack";
      CommandEnum2["ImportPlaylistTracks"] = "ImportPlaylistTracks";
      CommandEnum2["SearchSpotifyArtist"] = "SearchSpotifyArtistCommand";
      CommandEnum2["InsertLinkToSelectedPhoto"] = "InsertLinkToSelectedPhotoCommand";
      CommandEnum2["OpenLinkedPhoto"] = "OpenLinkedPhotoCommand";
      CommandEnum2["EnrichWithPromptUrl"] = "EnrichWithPromptUrlCommand";
      CommandEnum2["SyncContacts"] = "SyncContactsCommand";
      CommandEnum2["ToggleHideEmptyProperties"] = "ToggleHideEmptyProperties";
      CommandEnum2["TokenizeAndCreateDictionaryNotes"] = "TokenizeAndCreateDictionaryNotesCommand";
...
// src/Infrastructure/Obsidian/Views/GoogleAuthModal.ts
var GoogleAuthModal = class extends import_obsidian2.Modal {
  ...
  onOpen() {
    ...
    new import_obsidian2.Setting(step1).addButton((btn) => btn.setButtonText("Connect Google").setCta().onClick(async () => {
    ...
    let code = "";
    new import_obsidian2.Setting(step2).setName("Authorization Code").setDesc("Paste the code or the full redirect URL here.").addText((text) => text.setPlaceholder("Paste code or URL...").onChange((value) => {
      code = value.trim();
      try {
    ...
    new import_obsidian2.Setting(contentEl).addButton((btn) => btn.setButtonText("Verify & Connect").setCta().onClick(async () => {
      if (!code) {
...
var SyncGoogleContactsCommand = class {
  ...
  async execute(file) {
    ...
    if (!targetFile || !(targetFile instanceof import_obsidian4.TFile)) {
      new import_obsidian4.Notice("No hay ninguna nota activa.");
      return;
    }
    if (!this.isPersona(targetFile)) {
      new import_obsidian4.Notice("La nota activa no es una Persona (falta tag #Personas).");
      return;
    }
    const view = getActiveMarkdownView(this.app, targetFile);
    if (!view) {
      new import_obsidian4.Notice("No se pudo obtener la vista de la nota para activar el modo edici\xF3n.");
      return;
    }
    await executeInEditMode(view, async () => {
      new import_obsidian4.Notice(`Sincronizando con Google: ${targetFile.basename}...`);
      try {
        await this.plugin.syncContactUseCase.syncNoteWithGoogle(targetFile.path);
        new import_obsidian4.Notice(`Sincronizaci\xF3n con Google completada para ${targetFile.basename}`);
      } catch (e) {
        ...
        if (msg.includes("Refresh Token is missing") || msg.includes("No Google tokens") || msg.includes("401") || msg.includes("403") || msg.includes("invalid_grant") || msg.includes("Token has been expired")) {
          new import_obsidian4.Notice("Autenticaci\xF3n necesaria con Google.");
          new GoogleAuthModal(this.app, this.plugin.googleAdapter, async () => {
            new import_obsidian4.Notice("Autenticado correctamente. Ejecuta el comando de nuevo.");
          }).open();
        } else {
          new import_obsidian4.Notice(`Error al sincronizar con Google: ${msg}`);
        }
      }
    });
  }
...
var ProcessUnsyncedGoogleContactsCommand = class {
  ...
  async execute() {
    new import_obsidian5.Notice("Buscando contactos no sincronizados en Google...");
    try {
      const matches = await this.fetchUnsyncedContacts();
      if (matches.length === 0) {
        new import_obsidian5.Notice("No se encontraron contactos pendientes de sincronizar.");
        return;
      }
...
var UnsyncedContactsBatchModal = class extends import_obsidian5.Modal {
  constructor(app, matches, adapter, useCase) {
    super(app);
  ...
  renderContactItem(container, match) {
    ...
    new import_obsidian5.ButtonComponent(actionsDiv).setButtonText(match.suggestedNote ? "Enlazar" : "Enlazar...").setIcon("link").setTooltip(match.suggestedNote ? `Enlazar con ${match.suggestedNote.basename}` : "Buscar nota para enlazar").onClick(async () => {
      if (match.suggestedNote) {
    ...
    new import_obsidian5.ButtonComponent(actionsDiv).setButtonText("Crear").setIcon("file-plus").setCta().setTooltip("Crear nueva nota en Personas/Conocidos-mios").onClick(async () => {
      const newFilePath = await this.useCase.createNoteFromContact(match.contact);
      new import_obsidian5.Notice(`Nota creada: ${newFilePath}`);
      itemDiv.remove();
    });
    new import_obsidian5.ButtonComponent(actionsDiv).setButtonText("Mover").setIcon("import").setTooltip("Crear nota y eliminar de Google").onClick(async () => {
      if (confirm(`\xBFCrear nota para "${match.contact.name}" y eliminarlo de Google?`)) {
        ...
        if (match.contact.id) {
          ...
          new import_obsidian5.Notice(`Movido a Obsidian: ${match.contact.name}`);
        }
        itemDiv.remove();
      }
    });
    new import_obsidian5.ButtonComponent(actionsDiv).setButtonText("Eliminar").setIcon("trash").setWarning().setTooltip("Eliminar de Google Contacts").onClick(async () => {
      if (confirm(`\xBFSeguro que quieres eliminar a "${match.contact.name}" de Google Contacts?`)) {
        if (match.contact.id) {
          await this.adapter.deleteContact(match.contact.id);
          new import_obsidian5.Notice(`Eliminado de Google: ${match.contact.name}`);
        }
        itemDiv.remove();
      }
    });
...
var NoteSelectionModal = class extends import_obsidian5.FuzzySuggestModal {
...
var SettingsView = class extends import_obsidian6.PluginSettingTab {
  constructor(app, plugin) {
    super(app, plugin);
    this.plugin = plugin;
  }
  display() {
    ...
    new import_obsidian6.Setting(containerEl).setName("Google Client ID").setDesc("OAuth 2.0 Client ID").addText((text) => text.setPlaceholder("Enter your Client ID").setValue(this.plugin.settings.googleClientId).onChange(async (value) => {
      this.plugin.settings.googleClientId = value;
      await this.plugin.saveSettings();
    }));
    new import_obsidian6.Setting(containerEl).setName("Google Client Secret").setDesc("OAuth 2.0 Client Secret").addText((text) => text.setPlaceholder("Enter your Client Secret").setValue(this.plugin.settings.googleClientSecret).onChange(async (value) => {
...
// src/Infrastructure/Obsidian/ObsidianNoteRepository.ts
var import_obsidian7 = require("obsidian");
var ObsidianNoteRepository = class {
  constructor(app) {
    this.app = app;
  }
  async getNoteMetadata(path2) {
    ...
    return {
      ...
      tags: cache ? (0, import_obsidian7.getAllTags)(cache) || void 0 : void 0
    };
  }
  async readNote(path2) {
    const file = this.app.vault.getAbstractFileByPath(path2);
    if (!(file instanceof import_obsidian7.TFile))
      throw new Error(`File not found: ${path2}`);
    return await this.app.vault.read(file);
  }
  async appendContent(path2, content) {
    const file = this.app.vault.getAbstractFileByPath(path2);
    if (!(file instanceof import_obsidian7.TFile))
      throw new Error(`File not found: ${path2}`);
  ...
  async updateFrontmatter(path2, callback) {
    const file = this.app.vault.getAbstractFileByPath(path2);
    if (!(file instanceof import_obsidian7.TFile))
      throw new Error(`File not found: ${path2}`);
    await this.app.fileManager.processFrontMatter(file, callback);
  }
  async createNote(path2, content) {
    const file = await this.app.vault.create(path2, content);
    return {
      path: file.path,
      basename: file.basename,
      frontmatter: {},
  ...
  async getTemplateContent(path2) {
    const file = this.app.vault.getAbstractFileByPath(path2);
    if (file instanceof import_obsidian7.TFile) {
      return await this.app.vault.read(file);
    }
...
// src/Infrastructure/Obsidian/ObsidianUIService.ts
var import_obsidian8 = require("obsidian");
var ObsidianUIService = class {
  constructor(app) {
    this.app = app;
  }
  async selectContact(noteName, query, candidates) {
...
var ContactSelectionModal = class extends import_obsidian8.FuzzySuggestModal {
...
// src/Infrastructure/api/mappers/GoogleContactMapper.ts
var GoogleContactMapper = class {
  constructor(groupIdsToNames, groupNamesToIds) {
    this.groupIdsToNames = groupIdsToNames;
    this.groupNamesToIds = groupNamesToIds;
  }
  toDomain(person) {
    var _a2, _b, _c, _d, _e, _f;
    ...
    const groups = memberships.map((m) => {
      ...
      return id ? this.groupIdsToNames.get(id) : void 0;
    }).filter((g) => !!g);
    return {
      ...
      customFields: (person.userDefined || []).reduce((acc, curr) => {
...
var GoogleContactAdapter = class {
  ...
  async getAccessToken() {
    ...
    try {
      ...
      console.error("[GoogleAdapter] Auth Error", error);
      new import_obsidian9.Notice("Error al autenticar con Google. Revisa la consola.");
      throw error;
    }
  }
  async request(url, method = "GET", body) {
    ...
    const response = await (0, import_obsidian9.requestUrl)(params);
  ...
  async updateContact(resourceName2, contact) {
    ...
    if (contact.groups) {
      ...
      for (const m of currentMemberships) {
        ...
        const mappedName = id ? this.contactGroupIdsToNames.get(id) : void 0;
        if (!mappedName && id) {
          const exists = newMemberships.find((nm) => {
            var _a3;
            return ((_a3 = nm.contactGroupMembership) == null ? void 0 : _a3.contactGroupResourceName) === id;
          });
          if (!exists)
...
// src/Application/UseCases/SyncContactUseCase.ts
var SyncContactUseCase = class {
  constructor(noteRepo, uiService, adapter, transformer) {
   ...
   */
  async syncNoteWithGoogle(notePath) {
    ...
    await this.noteRepo.updateFrontmatter(notePath, (fm) => {
   ...
   */
  async linkContactToNote(contact, notePath) {
    await this.noteRepo.updateFrontmatter(notePath, (fm) => {
    ...
    this.uiService.notify(`Enlazado: ${contact.name} <-> ${meta == null ? void 0 : meta.basename}`);
  }
  /**
...
// src/Domain/GoogleContactTransformer.ts
...
var _GoogleContactTransformer = class _GoogleContactTransformer {
  ...
  updateFrontmatterFromContact(frontmatter, contact) {
    ...
    this.updateFieldIfMissing(frontmatter, import_core.FrontmatterKeys.Telefono, contact.phone);
    this.updateFieldIfMissing(frontmatter, import_core.FrontmatterKeys.Email, contact.email);
    this.updateFieldIfMissing(frontmatter, import_core.FrontmatterKeys.Cumpleanos, contact.birthday);
    this.updateFieldIfMissing(frontmatter, "aliases", contact.nickname ? [contact.nickname] : void 0);
    this.updateFieldIfMissing(frontmatter, import_core.FrontmatterKeys.Puesto, contact.jobTitle);
    this.updateFieldIfMissing(frontmatter, import_core.FrontmatterKeys.Empresa, contact.organization);
    this.updateFieldIfMissing(frontmatter, import_core.FrontmatterKeys.Direcciones, contact.addresses);
    this.updateFieldIfMissing(frontmatter, import_core.FrontmatterKeys.Urls, contact.urls);
    this.updateFieldIfMissing(frontmatter, import_core.FrontmatterKeys.Eventos, contact.events);
    this.updateFieldIfMissing(frontmatter, import_core.FrontmatterKeys.Relaciones, contact.relations);
    this.updateFieldIfMissing(frontmatter, import_core.FrontmatterKeys.Genero, contact.gender);
    this.updateFieldIfMissing(frontmatter, import_core.FrontmatterKeys.Ocupaciones, contact.occupations);
    this.updateFieldIfMissing(frontmatter, import_core.FrontmatterKeys.Intereses, contact.interests);
    this.updateFieldIfMissing(frontmatter, import_core.FrontmatterKeys.Habilidades, contact.skills);
    this.updateFieldIfMissing(frontmatter, import_core.FrontmatterKeys.Residencias, contact.residences);
    if (contact.customFields) {
   ...
   */
  toContactFromMetadata(basename, frontmatter, allFileTags) {
    ...
    return {
      ...
      phone: this.clean(this.toArray(frontmatter[import_core.FrontmatterKeys.Telefono])),
      email: this.clean(this.toArray(frontmatter[import_core.FrontmatterKeys.Email])),
      birthday: this.clean(frontmatter[import_core.FrontmatterKeys.Cumpleanos]),
      nickname: this.getStringFromCleaned(this.clean(frontmatter["aliases"])),
      jobTitle: this.clean(frontmatter[import_core.FrontmatterKeys.Puesto]),
      organization: this.clean(frontmatter[import_core.FrontmatterKeys.Empresa]),
      customFields: {
        eloSyncDate: (/* @__PURE__ */ new Date()).toISOString()
      },
...
var EloGoogleContactsPlugin = class extends import_obsidian10.Plugin {
  ...
  async onload() {
    ...
    this.googleAdapter = new GoogleContactAdapter(
      ...
      // Adapter needs saveSettings based on current impl
    );
