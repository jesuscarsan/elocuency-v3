...
export const FrontmatterTranslations: Record<string, Record<string, string>> = {
    es: {}, // Spanish is the default
    en: {
        ...
        Padres: "Parents",
        Pareja: "Partner",
        Exparejas: "Ex-partners",
        Hermanos: "Siblings",
        Familiares: "Relatives",
        CompanerosTrabajo: "Coworkers",
        Jefes: "Bosses",
        Empleados: "Employees",
        EstilosMusicales: "Musical styles",
...
export const FrontmatterKeys: Record<string, string> = { ...DefaultFrontmatterKeys };
...
export function buildFrontmatterRegistry(): Record<string, FrontmatterFieldConfig> {
    return {
        // Internal fields:
...
export const FrontmatterRegistry: Record<string, FrontmatterFieldConfig> = buildFrontmatterRegistry();

export function setFrontmatterLanguage(lang: string) {
    const translations = FrontmatterTranslations[lang] || FrontmatterTranslations['es'];

    for (const key of Object.keys(FrontmatterKeys)) {
        const defaultValue = (DefaultFrontmatterKeys as any)[key];
        if (defaultValue && defaultValue.startsWith('!!')) {
            FrontmatterKeys[key] = defaultValue;
        } else {
            FrontmatterKeys[key] = translations[key] || defaultValue;
        }
    }

    // Now rebuild the registry
    for (const key of Object.keys(FrontmatterRegistry)) {
        delete FrontmatterRegistry[key];
    }

    const newRegistry = buildFrontmatterRegistry();
    Object.assign(FrontmatterRegistry, newRegistry);
}
