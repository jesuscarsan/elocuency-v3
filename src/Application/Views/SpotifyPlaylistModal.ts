...
import { SpotifyAdapter, SpotifyPlaylist, SpotifyTrack } from '../../Infrastructure/Adapters/SpotifyAdapter/SpotifyAdapter';

export class SpotifyPlaylistModal extends Modal {
    private spotifyAdapter: SpotifyAdapter;
    private playlists: SpotifyPlaylist[] = [];
    private selectedPlaylist: SpotifyPlaylist | null = null;
    ...
    async onOpen() {
        ...
        try {
            this.playlists = await this.spotifyAdapter.getUserPlaylists();
    ...
    displayPlaylists() {
        ...
        this.playlists.forEach(playlist => {
            ...
            item.createDiv({ text: `${playlist.tracks.total} tracks`, cls: 'spotify-playlist-count' }).style.fontSize = '0.8em';
    ...
    async displayTracks(playlist: SpotifyPlaylist) {
        const { contentEl } = this;
        contentEl.empty();
        ...
        try {
            const tracks = await this.spotifyAdapter.getPlaylistTracks(playlist.id);
