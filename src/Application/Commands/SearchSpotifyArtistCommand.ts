...
import { SpotifyAdapter, SpotifyArtist } from "../../Infrastructure/Adapters/SpotifyAdapter/SpotifyAdapter";
import { SpotifyArtistSelectionModal } from "../Modals/SpotifyArtistSelectionModal";
...
export class SearchSpotifyArtistCommand implements Command {
    ...
    private async updateFrontmatter(file: TFile, artist: SpotifyArtist) {
        await this.app.fileManager.processFrontMatter(file, (frontmatter: any) => {
            frontmatter[FrontmatterKeys.SpotifyUri] = artist.uri;
            frontmatter[FrontmatterKeys.EstilosMusicales] = artist.genres;
            frontmatter[FrontmatterKeys.ImagenesUrls] = artist.images.map(img => img.url);
