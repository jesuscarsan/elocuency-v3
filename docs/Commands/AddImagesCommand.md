...
2.  **Verificación Previa**: Comprueba si la nota ya tiene imágenes en el campo `Imagenes urls` del frontmatter. Si ya tiene, se detiene para evitar duplicados.
3.  **Búsqueda de Imágenes**: Utiliza Google Custom Search para encontrar imágenes relacionadas con el título.
4.  **Actualización del Frontmatter**: Añade las URLs de las 3 primeras imágenes encontradas al campo `Imagenes urls`.
